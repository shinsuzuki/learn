#-------------------------------------------------------------------------------
# ConfirmJobFinishedSuccessfully.ps1: 正常に処理が終了しているかを確認(SQL Serverのjobエラーと処理時間 > 経路 )
#-------------------------------------------------------------------------------
Param(
    ...
    [parameter(mandatory)][String]$targertStrStep1,
    [parameter(mandatory)][String]$targertStrStep2,
    [parameter(mandatory)][String]$targertStrStep3
)
...
try {
    ...
    #-------------------- 対象文字列
    Write-Host "targertStrStep1:$targertStrStep1"
    Write-Host "targertStrStep2:$targertStrStep2"
    Write-Host "targertStrStep3:$targertStrStep3"
    ...
    foreach ($row in $table.Rows) {
        ...
        $targetStr = "";
        if ($stepId -eq 1) {
            $targetStr = $targertStrStep1;
        }
        elseif ($stepId -eq 2) {
            $targetStr = $targertStrStep2;
        }
        elseif ($stepId -eq 3) {
            $targetStr = $targertStrStep3;
        }
        else {
            Write-Host "sterpId error($stepId)"
            throw "sterpId error($stepId)"
        }

        $jobResult = "NG"
        if ($msg.Contains($targetStr)) {
            $jobResult = "OK"
        }

        $outmsg = "$jobResult,$runDateTimeStr,$stepId,$stepName,$runStatus,$runDuration,$sqlSeverity,$sqlMessageId,$msg";
        Write-Host $outmsg
